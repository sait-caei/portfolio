class CanvasManager 
{
	...
	setScale(x, y)
	{
		this.ctx.scale(x, y); console.log(x, y);
	}
	...
	drawRectangle(posStart, width, height, fillColor, strokeColor, strokeWidth)
	{		
		this.ctx.beginPath();	
		this.ctx.rect(posStart.x, posStart.y, width, height);
	...
	/*drawOval(pos, radius, xScalePercent, yScalePercent, fillColor, strokeColor, strokeWidth)
	{
		...
		this.ctx.arc(pos.x/xScalePercent, pos.y/yScalePercent, radius, 0 * Math.PI, 2 * Math.PI);
		this.drawFill(fillColor);		
		this.drawStroke(strokeColor, strokeWidth);	
		this.ctx.closePath();
		this.ctx.restore();
